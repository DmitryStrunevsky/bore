...
const CONTROL_PORT: u16 = 12345; // Replace 12345 with the actual control port value
...
async fn spawn_server(secret: Option<&str>) {
    tokio::spawn(Server::new(1024..=65535, secret, 1600).listen());
...
async fn spawn_client(secret: Option<&str>) -> Result<(TcpListener, SocketAddr)> {
    ...
    let client = Client::new("localhost", local_port, "localhost", 0, secret, 1600).await?;
...
fn empty_port_range() {
    let min_port = 5000;
    let max_port = 3000;
    let _ = Server::new(min_port..=max_port, None, 1600);
}
